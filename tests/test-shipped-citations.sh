...
# The project-root files install.sh writes but does NOT copy from the payload — derived from its own
# receipt-row `printf` formats rather than listed, so a new one is covered the day it lands.
#
# HOISTED ABOVE RULE 2 ON 2026-08-16, and the reason is a re-examination rather than a convenience.
# Rule 2 resolved a token against `PAYLOAD` alone while rule 4, four hundred lines below, already
# derived this second half — so the two rules disagreed about what "an installed reader can open",
# and rule 2's own escape hatch, `CITATION_ALLOW`, carried `CLAUDE.md` by hand for exactly the case
# this derivation covers. That list says of itself *"if this list grows, rule 2 is wrong —
# re-examine it, do not append"*, and `AGENTS.md` is what made it grow: the Codex entry document is
# generated into a project the same way `CLAUDE.md` is, this repository happens to track one of its
# own, and a Codex session has to be told the file's name. Appending would have been the third
# member of a list whose own comment forbids a third member.
#
# THE FAILURE DIRECTION IS SAFE. If the extraction stops matching, this set goes empty, rule 2 flags
# MORE citations rather than fewer, and rule 4's own floor (`>= 2`, four hundred lines down) reds
# with the diagnosis. A derivation that can only over-report is one rule 2 may consume before that
# floor has run.
...
#              Generated rather than receipted under its own name, so `UP_ROOT_INSTALLED` (which
#              reads receipt-row formats) does not carry it and the entry stays. Its sibling
#              `AGENTS.md` — the Codex entry document — IS receipted, and is therefore resolved by
#              the derivation above rather than added here. That is the re-examination this comment
#              demands, run on 2026-08-16 when `AGENTS.md` would have been the fourth member.
...
while IFS= read -r md; do
  ...
  while IFS= read -r ln_tok; do
    ...
    # …and a project-root file install.sh WRITES is equally openable by an installed reader, even
    # though no payload entry carries it. See the derivation's header above.
    if grep -qxF -- "$t" <<< "$UP_ROOT_INSTALLED"; then continue; fi
...
# receipt rows for. `UP_ROOT_INSTALLED` is derived from install.sh's own row-writing printf formats
# rather than listed -- `.mcp.json` and `MCP-SETUP.md` are installed and live nowhere near
# `.claude/`, so a payload-only test would demand a `not installed` marker on two files that are.
# It is computed once, beside `PAYLOAD` at the top of this file, because rule 2 consumes it too;
# the header there explains why the hoist was a re-examination rather than a tidy-up.
