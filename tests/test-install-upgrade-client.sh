...
#
# THAT LAST SENTENCE WAS HOST-CONDITIONAL FOR ONE COMMIT, AND SAYING SO IS THE
# POINT OF KEEPING IT. `mv` is `rename(2)` only within one filesystem; the temp
...
# READ BEFORE THE RESTORE, because the restore is what would hide it.
TIUC_MODE16="$(stat -c '%a' "$P16/.codex/hooks.json" 2>/dev/null || echo unknown)"
...
# THE ROW ABOVE CANNOT SEE THE REFUSAL, AND THIS ONE CAN. Measured 2026-08-17 with the refusal made
# unreachable: the byte comparison still PASSED, for two independent reasons — install 2 regenerates
# byte-identical content from the same settings.json, and `mv` onto a 0444 file in a writable
# directory SUCCEEDS (`mv rc=0, content=NEW, mode=644` on a standalone primitive). So the strongest-
# sounding assertion in this arm was satisfied by a run that overwrote the file, and only the message
# row detected the missing refusal. The MODE is the discriminator the bytes cannot be: a refusal
# leaves 444, and every way of writing that file leaves 644.
tiuc_eq "444" "$TIUC_MODE16" \
  "7i: …and it is still 0444, which is the only observation here that a rewrite could not also produce — the byte comparison above passes over a run that replaced the file, because the regenerated bytes are identical and mv onto a read-only file in a writable directory succeeds at mode 644"
...
# rc 1 — the write cannot be completed. A read-only PARENT is the portable,
# tty-free way to produce that.
#
# WHICH STATEMENT FAILS MOVED ON 2026-08-17, AND THE ROWS DID NOT. The temp used
# to be `"$2.merged"` — beside the FACTS file, which every caller `mktemp`s, so it
# lived in `$TMPDIR` and only the `mv` failed here. That made the function's own
# promise — "returns non-zero without touching the target" — true only while
# `$TMPDIR` and the project shared a filesystem; across one, `mv` copies, and an
# interrupted copy leaves the TARGET truncated. The temp is now derived from the
# TARGET, so the rename is a rename on every host and, on this fixture, the awk
# redirection is what the sealed directory refuses. Same status, same untouched
# target, one statement earlier — which is why both rows below still read the way
# they did, and why neither needed editing when the mechanism changed.
tiuc_m8_reset
