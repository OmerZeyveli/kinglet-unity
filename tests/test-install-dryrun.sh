...
#     THE REFRESH BRANCH IS THE PATH THAT KEEPS FALLING INTO THIS, and `stale_region` below is how
#     the two fixtures that take it are kept out of it: it makes the region genuinely out of date, so
#     the refresh genuinely changes bytes and the promise becomes checkable. Until 2026-08-14 they
#     stayed out of it by ACCIDENT — the merge awk added a blank line on every run — and the accident
#     was itself a defect.
...
# stale_region <project-dir> — put one line inside CLAUDE.md's generated region that the next
# refresh must remove.
#
# WHY EVERY REFRESH-BRANCH FIXTURE NEEDS THIS, AND WHY IT IS NOT DECORATION. This file's own fixture
# rule, stated in the header, is that no promised project-root path may be rewritten with its own
# bytes: oracles 1 and 3 read the filesystem and cannot tell that rewrite apart from a broken
# promise. The refresh branch used to satisfy that rule BY ACCIDENT — its merge awk printed a blank
# line before the end marker on every run, so `upgrade` and `mcpthen` showed CLAUDE.md as changed and
# the promise read as kept. Task 4b removed that line, because the refresh arm was emitting the
# region one line longer than the fresh arm it is supposed to reproduce, and both fixtures failed
# that instant with `the dry run promised CLAUDE.md and the real run neither created nor changed it`
# — on a run that had refreshed the file correctly.
#
# So the staleness is supplied deliberately instead of relied on. It is the more faithful upgrade
# anyway: a project reinstalled after its Unity version, its packages or the toolkit moved has a
# region whose contents really are out of date, which is the only reason the refresh exists. And it
# raises the claim from "the file changed" to "the region was regenerated", which is what the dry
# run's line actually promises.
#
...
i = next(n for n, l in enumerate(lines) if 'kinglet:generated:begin' in l)
...
# Before the probe: probe's `before` snapshot is taken after this setup, so the stale line has to be
# in the file by now or the change it causes is invisible to oracle 3.
stale_region "$F_UPGRADE"
probe upgrade "$F_UPGRADE" upgrade
assert_region_refreshed "upgrade" "$F_UPGRADE"
...
stale_region "$F_MCPTHEN"
probe mcpthen "$F_MCPTHEN" upgrade
assert_region_refreshed "mcpthen" "$F_MCPTHEN"
if [ -f "$F_MCPTHEN/$WITHMCP_BAK" ]; then
