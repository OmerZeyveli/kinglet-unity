...
2 0000 - KNOWN FALSE POSITIVE, residual and disclosed: the routing precondition is a RAW-STRING test, so a routing WORD sitting in DATA routes exactly as one in command position does. A pure append whose message happens to name xargs, blocked at 2 before the precondition and 2 after - narrowed, not closed. It fired on a reviewer, after the fix, on the first command of the review. Closing it means deciding the route from tokens rather than from the raw string
echo "$(date -u) xargs Assets/Player.cs.meta" >> audit.log
2 0000 - THE -exec HALF OF THE ROUTING PRECONDITION, which nothing else in this file measures: deleting that alternative alone left every other assertion green. Two properties make this payload the discriminator and both are load-bearing - gfind is GNU find under the spelling a BSD host uses and does not match the find boundary (the preceding g is inside the excluded class), and the exec'd command must be READ-ONLY, because a writing one is classified by the META_EXEC arm before the unparseable marker is ever reached and blocks with the alternative deleted, proving nothing
gfind Assets -name '*.meta' -execdir grep -l "$(printf guid)" {} \;
