...
|---|---|---|
| "The suite passed before I made this change" | A green run only proves the tree it ran on, not the one you are about to ship. | A commit recorded as passing that did not pass its own suite: the run that proved it was taken on a different tree, before the change it was proving |
...
| "The test is green, so the test is real" | A test that asserts nothing passes every time. Watch it fail first, or you are trusting a test you have never seen fail. | A test file that inherits its assertion helpers from the runner exits 0 having asserted nothing when run on its own — green every time, about nothing |
