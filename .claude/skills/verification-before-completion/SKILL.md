...
| "The reviewer confirmed it" | Confirm what it verified *against*. A review that checks a string for internal consistency with the repo's own convention can bless a string the live system does not accept. | The `mcp__UnityMCP__` casing: a whole-branch review declared it wrong from `install.sh` and eight existing references, and had it changed to the differently-broken `mcp__unityMCP__` — the live server is `UnityMCP` |
