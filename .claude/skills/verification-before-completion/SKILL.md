...
| "It compiles, so it works" | Compilation is not behaviour, and in Unity it is not even proof of a valid file — the editor can write files that fail to compile without the write itself failing. | `unity-specifics.md`'s editor/runtime guard |
| "I described the manual Editor step, so it is handled" | A described step nobody performed is a step that did not happen. The instructions are not the atlas. | `performance.md`'s Developer Action Items |
