...
for src in "$CMD_DIR"/*.md; do
...

  {
    ...
    printf '%s\n' '> for you — you are reading it because you chose to. Three things in the body below name'
    printf '%s\n' '> Claude Code mechanisms that do not exist here, and each has a stated fallback:'
    printf '%s\n' '>'
    ...
    printf '%s\n' '> - **`CLAUDE.md`.** In this project the injected entry document is `AGENTS.md`. It is'
    printf '%s\n' '>   generated in the same run from the same project scan, but it is NOT a copy: the two'
    printf '%s\n' '>   fork wherever a statement is client-conditional, and `CLAUDE.md`'"'"'s side of each fork'
    printf '%s\n' '>   is the stronger claim. It is present and readable; it is simply not loaded for you,'
    printf '%s\n' '>   and it is the other client'"'"'s document.'
    printf '%s\n' '> - **Slash commands the body tells you to offer or run.** The first sentence above'
    printf '%s\n' '>   says there is no `/name` to type, and the body below still names them — every one'
    printf '%s\n' '>   of Kinglet'"'"'s nine commands, including this one, is installed here as a skill of the'
    printf '%s\n' '>   same name. **Translate rather than skip:** read `.agents/skills/<name>/SKILL.md`'
    printf '%s\n' '>   and follow it. Where the body says to *offer* one to the user, offer the work, not'
    printf '%s\n' '>   the keystroke.'
    printf '%s\n' '>'
