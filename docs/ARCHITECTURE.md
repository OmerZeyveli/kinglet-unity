...
scripts/             Shell scripts: the two surviving validators (serialization, assembly
                     definitions), the pipeline detector, the CLAUDE.md / AGENTS.md generator,
                     the doctor, the dangling-reference sweep, this repo's provenance check,
                     and the three Codex-client scripts (the hook shim, the command-to-skill
                     converter, and the probe harness — the last is repo-only and not shipped)
install.sh           One-command installer, `--client claude` (default) or `--client codex`
...
AGENTS.md            This repository's entry document for a Codex CLI session (see below)
```
...
*Measured against `codex-cli 0.145.0`, 2026-08-15/16, on linux-x64. Per-class verdicts with the
command behind each are in `docs/research/codex-client/findings.md`; Codex's own capabilities are in
`codex-facts.md`. **Nothing in this section is inferred from parity with Claude Code.***

### The shape, and why it is not a second payload

**One content tree, one runtime adapter, everything client-specific generated at install time and
none of it tracked.** `.claude/` stays the single hand-authored surface — `.claude/hooks/` and
`.claude/settings.json` were not touched by the Codex wave at all — and the Codex layer is produced
from it on the target machine:

```
<project>/AGENTS.md               generated by scripts/generate-claude-md.sh --client codex
<project>/.agents/skills/         a real directory of per-entry symlinks into .claude/skills/,
                                  plus one generated SKILL.md per .claude/commands/*.md
<project>/.codex/hooks.json       generated by .claude/scripts/codex-hook-shim.sh --emit-config
<project>/.codex/config.toml      the Unity bridge row
...
`content`, `new_string` and `old_string`, none of which exist there, so eight of the nine tool-event
hooks match, run, and exit 0 having done nothing. `scripts/codex-hook-shim.sh` normalises the
envelope into the shape the hooks already read and runs one hook per file the patch touches; the
hooks themselves are unchanged, which is why one implementation still has one test surface behind it.
No file layout fixes this, which is why the repository's platform-build machinery (`src/catalog/`,
`tools/kinglet_build/`, `adapters/*/profile.json`) was not used and not extended — see *the debt this
decision strands* in `findings.md`.
...
| Agents | **excluded** | Codex has no **per-agent** capability contract, so a read-only reviewer would arrive able to write. Read-only itself exists — `codex --sandbox read-only`, `sandbox` on thread start, `sandboxPolicy` on turn start (each parameter carries exactly one of the two, and they are different types), `permissionProfile/list` — and so does a sub-agent lifecycle (`subagentStart` / `subagentStop` are hook events). What has no expression is attaching a capability set to a *named agent definition*, which is the whole content of the `tools:` key |
...
registered at all, and nothing says so.** There is no diagnostic for a user or for
`studio-doctor.sh` to key on, and it is the state a correct install is in until `codex` has been run
once in the project. It is the sixth of six measured silent-failure layers, all of which are
enumerated in one table in `findings.md`; it is named here because it is the one a real user meets
first.
