...
| `T5` is classified as a floor, but the reviewer judges the label **generous**: it is unobservable alone *and* combined, 0/60 in a race probe. The sentinel split converted its defect into a no-op | **CLOSED 2026-08-17 by Task 13 — the subject is absent.** Task 11 never visited `tests/test-codex-shim.sh` and the whole-branch review recorded that; re-checked today, `grep -n 'T5' tests/test-codex-shim.sh` returns **nothing**, so there is no label left to be generous. The substantive half is discharged by the tree rather than by an edit: `findings.md`'s canonical six-layer table carries no `T5` row, and its layer 5 is *the hook's own ceiling*, which is measured and has its own probe. Nothing is owed; a reader who wants the shape re-litigated opens a new task with a measurement | **Closed — no owner** |
...
| The `HookEventName` enum check stays undone. The decline is right; its stated dichotomy was false and is now corrected — the suite **already skips with a stated reason**, so deriving the enum only when `codex` is on `PATH` would add no dependency and hardcode nothing | **CLOSED 2026-08-17 by Task 13 — recorded by design, and the record is complete.** It was never a deferral: `codex-facts.md` carries the full 11-member enum verbatim, and `tests/test-codex-surface.sh` carries the option, both real reasons it is undone (the binary dependency, and the stale-list-inside-a-guard failure) **and** the named non-reason, so the next reader inherits a decision rather than a gap. The residual is one reviewed edit wide and the identity above it narrows drift to exactly that edit. Writing the check is a new task with its own measurement, not the discharge of this one | **Closed — no owner** |
