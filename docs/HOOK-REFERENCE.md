...
- **Scope:** the **third-party skip list is shared** with `block-legacy-input` and is identical on purpose -- the pair disagreeing about what that list means is what let the unanchored `Packages/` and `Library/` entries survive a wave after being written down. Same four vendored trees anchored under `Assets/`, same `Library/PackageCache/` skip, same `Assets/`-segment gate. See that hook's Scope note above.
- **Scope, the deliberate divergence:** the two hooks are **not** identical, and this line read *"identical to `block-legacy-input`'s"* until 2026-08-15. `block-legacy-input.sh` carries a **second** `case` that its sibling has no equivalent of, skipping `*/Assets/Editor/*`, `*/Assets/*/Editor/*`, `*/Assets/Tests/*` and `*/Assets/*/Tests/*`. **This hook warns on editor and test code, and that is correct.** Legacy `Input.*` is a *runtime* concern: an `Editor/` or `Tests/` asmdef sets `"includePlatforms": ["Editor"]` and is excluded from builds entirely, so the API it calls never ships. A `#if UNITY_PS5` with **no** `#else`, by contrast, silently excludes its block on every other platform *wherever the file lives* -- an editor tool that compiles only under one console define is a real defect in the Editor itself. **Do not "restore consistency" by copying the Editor/Tests skip across.** Doing so narrows a live hook onto a class of files where the failure it detects is still real, and the suite would stay green: `tests/test-hook-behaviour.sh` probes the shared third-party skip, not this divergence.
- **Environment variables:** None
